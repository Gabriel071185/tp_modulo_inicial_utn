...
def create_table_db(fd): 
    cursor = fd.cursor()
    sql = "CREATE TABLE sisgesres(id INTEGER PRIMARY KEY AUTOINCREMENT, mesa INTEGER NOT NULL, pedido TEXT, total INTEGER NOT NULL)"
...
def alta_db(fd):
    ...
    global diccionario
    global suma
    datos = (int(mesa), json.dumps(diccionario), int(suma))
...
'''def consulta_particular(fd):
    cursor = fd.cursor()
    cursor.execute("SELECT pedido FROM sisgesres")
    rows = cursor.fetchall()
    
    for row in rows:
        pedido = json.loads(row[0])  # Convertir el texto del pedido a un diccionario de Python
        for comida, cantidad in pedido.items():
            if comida == "pollo":
                var_comida1.set(cantidad)
            elif comida == "carne":
                var_comida2.set(cantidad)
            # Agrega el resto de las comidas según corresponda
        # Agrega el resto de las comidas según corresponda
'''

def consulta_particular(fd, cursor):
    cursor.execute("SELECT pedido FROM sisgesres")
    rows = cursor.fetchall()
    
    for row in rows:
        try:
            pedido = json.loads(row[0])
            print(f"tipo-> {type(pedido)} \nDiccionario? -> {pedido}")
            ...
            print(f"Error de formato JSON en el pedido: {e}")


'''def consulta_particular(fd, grilla):
   # global diccionario 
    ...
    cursor = fd.cursor()
    data = (id_db, )
    sql = "SELECT * FROM sisgesres WHERE id = ?;"
    cursor.execute(sql, data)

    print(f" VALORES TRAIDOS DE LA DB :D ----> {type(row['values'][1])}")
    print(f" VALORES TRAIDOS DE LA DB :D ----> {(row['values'][1])}")'''
    

""" # {'pollo' : [3, 2900]}
    seleccion = {}

    for keys, valores in (dict(row['values'][1])).items():
        if valores[0] != 0:
            seleccion.update({keys : valores})
            print(f"\n\nDICCIONARIO DE SELECCION -> {seleccion}")
    var_mesa.set("El pedido seleccionado es " + seleccion)  """
...
def alta():
    ...
    for key, value in diccionario.items():
        if value[0] > 0:
           seleccion.append((mesa, key, value[0], value[1], suma)) 
...
def modificar_db(fd, id_db, data1, data2, data3, data4, name="tabla1"):
    cursor = fd.cursor()
    sql = f"UPDATE {name} SET nombre = ?, apellido = ?, pais = ?, edad = ? WHERE id = ?;"

    datos = (str(data1), str(data2), str(data3), int(data4), int(id_db))

    cursor.execute(sql, datos)
    fd.commit()
    
        for row in lista_logs:
            #print(f"{row['nombre']} --- {row['apellido']} --- {row['pais']} --- {row['edad']}")
            alta_db(fd_base, data1=row["nombre"], data2=row["apellido"], data3=row["pais"], data4=row["edad"], name=name_table)
"""
...
def reset_menu():
    ...
    global suma
    suma = 0
    var_mesa.set("Borrón y cuenta nueva!")
...
pantalla_registro = tk.Label(panel_menu1, textvariable=var_mesa , font=("Arial", 15), bg=c_hueso, fg=c_marron_osc, bd=3, relief="raised")
...
cursor = fd_base.cursor()
boton_consulta = tk.Button(panel_botones, bg=c_marron, fg=c_naranja, text="Consulta", font=("Montserrat", 15, "bold"), relief="raised",bd=5, command=lambda: consulta_particular(fd_base, cursor))
